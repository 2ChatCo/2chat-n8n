...
export async function router(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
	...
	for (let i = 0; i < items.length; i++) {
		...
		try {
			...
			} else if (twochat.resource === 'waba') {
				if (twochat.operation === 'send') {
					responseData = await waba.send.execute.call(this, i);
				} else if (twochat.operation === 'sendTemplate') {
					responseData = await waba.sendTemplate.execute.call(this, i);
				} else if (twochat.operation === 'sendInteractive') {
					responseData = await waba.sendInteractive.execute.call(this, i);
				}
			}
