...
export class TwoChatTrigger implements INodeType {
    ...
    properties: [
      {
        ...
        options: [
          ...
          { name: 'New Message Sent or Received', value: 'whatsapp.message.new' }
        ],
  ...
  async webhook(this: IWebhookFunctions): Promise<IWebhookResponseData> {
    ...
    // if (headerAuth) {
    //   // Implement signature validation if 2Chat provides this
    // }

    let body = req.body;

    // Add metadata to help with workflow processing
    if (body) {
      // Add timestamp for when we received the webhook
      body = {
        ...body,
